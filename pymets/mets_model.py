...
def initialise_values(element, attribs_list):
    for key in element.attrib:
        if key in attribs_list:
            if key in ["href", "arcrole", "title", "show", "acutate", "to", "FROM"]:
                element.set("{%s}%s" % (XLIN_NS, key), element.attrib[key])
                del element.attrib[key]
        elif key not in attribs_list:
            if hasattr(element, "TAG"):
                print("WARN: {} not allowed in element {}".format(key, element.TAG))
    ...
    def write(self, filename, pretty_print=False, encoding="unicode"):
        if encoding in ["unicode"]:
            with open(filename, "w") as f:
                f.write(ET.tostring(self, pretty_print=pretty_print, encoding=encoding))
        else:
...
class MetsHdr(ET.ElementBase):
    ...
    def _init(self):
        initialise_values(
            self, ["ID", "ADMID", "CREATEDATE", "LASTMODDATE", "RECORDSTATUS"]
        )
...
class Stream(ET.ElementBase):
    ...
    def _init(self):
        ...
        initialise_values(
            self,
            ["ID", "OWNERID", "ADMID", "DMDID", "BEGIN", "END", "BETYPE", "streamType"],
        )
...
class SmLink(ET.ElementBase):
    ...
    def _init(self):
        initialise_values(
            self, ["ID", "arcrole", "title", "show", "actuate", "to", "from"]
        )
...
class Behavior(ET.ElementBase):
    ...
    def _init(self):
        initialise_values(
            self, ["ID", "STRUCTID", "BTYPE", "CREATED", "LABEL", "GROUPID", "ADMID"]
        )
